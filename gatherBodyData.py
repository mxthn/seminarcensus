	...
	def run(self):
		...
		for line in scandata:
			if self.searchstring.lower() in line.lower():
				...
				response = lineSplit[3]
				unquotedresponse = urllib.unquote(lineSplit[3].replace("=","%"))

				m = re.search(serverflagregex, unquotedresponse, re.I)
				...
				if m and len(m.group()) >= 1:
					...
					if self.searchpattern.search(serverflag):					
						...
						htmlresponse = re.search(bodyregex, response, re.I)
						
						if htmlresponse and len(htmlresponse.group()) >= 1:

							responselist =  self.evalDictionary[ip]

							responselist.append("<html>"+htmlresponse.group(1)+"</html>")
							print "Added Ip response to Dictionary for ip" + ip
	...
	def writeResults(self):
		...
		if f:
			for key in self.evalDictionary:
				responselist = self.evalDictionary[key]
				f.write(key)
